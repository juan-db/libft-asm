...
int memset_test_compare_memset()
{
	...
	{
		fprintf(stderr, "\033[31mft_memcmp does not match the behaviour of memcmp with input [char[16], 'c', 16]\n");
...
int memset_test_compare_memset_larger_than_char_value()
{
	...
	if (memcmp((void*)expected, (void*)actual, 16))
	{
		fprintf(stderr, "\033[31mft_memcmp does not match the behaviour of memcmp with input [char[16], 0x5044, 16]\n");
		fprintf(stderr, "Expected: ");
		print_array(stderr, expected, 16);
		fprintf(stderr, "Actual: ");
		print_array(stderr, actual, 16);
...
int memset_test_compare_memset_zero_nbyte()
{
	...
	if (memcmp((void*)expected, (void*)actual, 16))
	{
		fprintf(stderr, "\033[31mft_memcmp does not match the behaviour of memcmp with input [char[16], 'c', 0]\n");
		fprintf(stderr, "Expected: ");
		print_array(stderr, expected, 16);
		fprintf(stderr, "Actual: ");
		print_array(stderr, actual, 16);
